    ...
    if (await enableButton.first().isVisible()) {
      ...
      if (await codeInput.isVisible()) {
        ...
        if (await verifyBtn.last().isVisible()) {
          ...
          const _hasError = await error.first().isVisible().catch(() => false);
...
test.describe('MFA - Backup Codes', () => {
  ...
  test('should show backup codes section for MFA-enabled users', async ({ page }) => {
    ...
    const _hasBackupSection = await backupSection.first().isVisible().catch(() => false);
    const _hasRegenerateBtn = await regenerateBtn.isVisible().catch(() => false);
...
test.describe('MFA - Disable', () => {
  ...
  test('should have disable MFA option when MFA is enabled', async ({ page }) => {
    ...
    const _hasDisableBtn = await disableBtn.first().isVisible().catch(() => false);
    const _hasToggle = await mfaToggle.first().isVisible().catch(() => false);
...
test.describe('Security - Sessions Management', () => {
  ...
  test('should show sign out all sessions option', async ({ page }) => {
    ...
    const _hasSignOutAll = await signOutAllBtn.isVisible().catch(() => false);
