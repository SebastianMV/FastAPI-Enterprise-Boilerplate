...
describe('usersService', () => {
  ...
  describe('list', () => {
    ...
    it('should pass pagination params', async () => {
      mockGet.mockResolvedValueOnce({ data: { items: [], total: 0, skip: 10, limit: 5 } });
      await usersService.list({ skip: 10, limit: 5 });
      expect(mockGet).toHaveBeenCalledWith('/users', { params: { skip: 10, limit: 5 } });
    });
  });
  ...
  describe('update', () => {
    it('should PATCH /users/:id', async () => {
      mockPatch.mockResolvedValueOnce({ data: { id: 'u1', first_name: 'Updated' } });
      const result = await usersService.update('u1', { first_name: 'Updated' });
      expect(mockPatch).toHaveBeenCalledWith('/users/u1', { first_name: 'Updated' });
  ...
  describe('uploadAvatar', () => {
    it('should POST /users/me/avatar with FormData', async () => {
      ...
      mockPost.mockResolvedValueOnce({ data: { id: 'me', avatar_url: '/avatars/me.png' } });
