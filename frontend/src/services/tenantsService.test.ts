...
describe("tenantsService", () => {
  ...
  it("should list tenants", async () => {
    ...
    const result = await tenantsService.list({ page: 1, page_size: 20 });
  ...
  it("should update plan", async () => {
    ...
    const result = await tenantsService.updatePlan(
      "t1",
      "premium",
      "2027-01-01",
    );
    expect(mockPatch).toHaveBeenCalledWith("/tenants/t1/plan", {
      plan: "premium",
