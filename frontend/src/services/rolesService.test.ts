...
describe("rolesService", () => {
  ...
  it("should list roles", async () => {
    ...
    const result = await rolesService.list({ page: 1, page_size: 10 });
  ...
  it("should create a role", async () => {
    ...
    mockPost.mockResolvedValueOnce({
      data: { id: "r2", ...data, is_system: false },
    });
  ...
  it("should assign role to user", async () => {
    ...
    const result = await rolesService.assignToUser({
      user_id: "u1",
      role_id: "r1",
    });
    expect(mockPost).toHaveBeenCalledWith("/roles/assign", {
      user_id: "u1",
      role_id: "r1",
    });
  ...
  it("should revoke role from user", async () => {
    ...
    const result = await rolesService.revokeFromUser({
      user_id: "u1",
      role_id: "r1",
    });
    expect(mockPost).toHaveBeenCalledWith("/roles/revoke", {
      user_id: "u1",
      role_id: "r1",
    });
