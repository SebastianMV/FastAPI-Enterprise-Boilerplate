  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        setStatus("success");
        setMessage(
          is_new_user ? t("oauth.accountCreated") : t("oauth.signedIn"),
        );
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- avoid re-running OAuth callback flow on translation function identity changes
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900">
      <div className="card p-8 max-w-md w-full text-center">
        ...
        <h1 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">
          ...
          {status === "success" &&
            (isNewUser ? t("oauth.welcome") : t("oauth.welcomeBack"))}
        ...
        {status === "success" && (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {t("oauth.redirecting", {
              destination: isNewUser
                ? t("oauth.yourProfile")
                : t("oauth.theDashboard"),
            })}
          </p>
        )}
      </div>
