...
describe("TenantsPage", () => {
  ...
  it("shows plan badges", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("tenants.plans.professional"),
      ).toBeInTheDocument();
    });
  ...
  it("deactivates active tenant from action button", async () => {
  ...
  it("activates inactive tenant from action button", async () => {
    const user = userEvent.setup();
    renderPage();
  ...
  it("opens settings modal from tenant card action", async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    const settingsButtons = screen.getAllByTitle("tenants.viewSettings");
    await user.click(settingsButtons[0]);

    await waitFor(() => {
      expect(screen.getByText("tenants.tenantSettings")).toBeInTheDocument();
      expect(screen.getByText("tenants.settings.maxUsers")).toBeInTheDocument();
    });
  });

  it("updates tenant from edit modal submit", async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    const editButtons = screen.getAllByTitle("tenants.editTenant");
    ...
    await user.click(
      screen.getByRole("button", { name: "tenants.saveChanges" }),
    );
  ...
  it("deletes tenant from delete confirm modal", async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    const deleteButtons = screen.getAllByTitle("tenants.deleteTenant");
