...
for idx, it in enumerate(issues, start=1):
    full_rows.append(
        f"| {idx} | {it['severity']} | `{it['rule']}` | `{it['file']}` | {it['line']} | {it['message'].replace('|', '\\|')} | `{it.get('snippet', '').replace('|', '\\|').replace(chr(10), ' ')}` |"
    )

(out_dir / "AUDIT39_ISSUES_TABLE_FULL.md").write_text(
    "# Audit N°39 - Tabla Completa de Issues\n\n"
...
(out_dir / "AUDIT39_ISSUES_TABLE_SUMMARY.md").write_text(
    "# Audit N°39 - Resumen por Severidad\n\n"
...
(out_dir / "AUDIT39_ISSUES_TABLE_BY_RULE.md").write_text(
