...
from app.cli.utils import format_uuid, confirm_action
...
async def _create_api_key(
    ...
    async with async_session_maker() as session:
        ...
        if not user:
            console.print(f"[red]User not found: {user_email}[/red]")
            raise typer.Exit(1)
        ...
        console.print(f"  Scopes: {', '.join(scopes) if scopes else 'All (no restrictions)'}")
        console.print(f"  Expires: {expires_at.strftime('%Y-%m-%d') if expires_at else 'Never'}")
        console.print()
...
def list_api_keys(
    ...
    show_inactive: bool = typer.Option(False, "--inactive", "-i", help="Include inactive keys"),
) -> None:
    """
    List all API keys.
    ...
    async with async_session_maker() as session:
        ...
        for api_key, user in keys:
            ...
            table.add_row(
                ...
                api_key.prefix + "...",
                user.email,
                ", ".join(api_key.scopes[:2]) + ("..." if len(api_key.scopes) > 2 else "") if api_key.scopes else "*",
                status,
                api_key.expires_at.strftime("%Y-%m-%d") if api_key.expires_at else "Never",
                api_key.last_used_at.strftime("%Y-%m-%d") if api_key.last_used_at else "Never",
            )
...
async def _revoke_api_key(key_id: str, force: bool) -> None:
    ...
    async with async_session_maker() as session:
        ...
        if not api_key.is_active:
            console.print(f"[yellow]API key is already revoked: {api_key.name}[/yellow]")
            raise typer.Exit(0)
        
        if not force:
            if not confirm_action(f"Revoke API key '{api_key.name}' ({api_key.prefix}...)?"):
                raise typer.Exit(0)
...
async def _api_key_info(key_id: str) -> None:
    ...
    async with async_session_maker() as session:
        ...
        console.print()
        console.print(f"  Status:      {'[red]Revoked[/red]' if not api_key.is_active else ('[yellow]Expired[/yellow]' if is_expired else '[green]Active[/green]')}")
        console.print(f"  Created:     {api_key.created_at.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        console.print(f"  Expires:     {api_key.expires_at.strftime('%Y-%m-%d %H:%M:%S UTC') if api_key.expires_at else 'Never'}")
        console.print()
        console.print(f"  Scopes:")
        if api_key.scopes:
            for scope in api_key.scopes:
                console.print(f"    - {scope}")
        else:
            console.print(f"    [dim](All permissions)[/dim]")
        console.print()
        console.print(f"  Usage:")
        console.print(f"    Total requests:  {api_key.usage_count}")
        console.print(f"    Last used:       {api_key.last_used_at.strftime('%Y-%m-%d %H:%M:%S UTC') if api_key.last_used_at else 'Never'}")
