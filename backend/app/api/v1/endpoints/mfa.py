...
async def get_mfa_config(user_id: str) -> MFAConfig | None:
...
async def save_mfa_config(config: MFAConfig) -> None:
    """Save MFA config to Redis (no TTL — MFA persists indefinitely)."""
    cache = await _get_redis()
