...
class I18n:
    ...
    def t(
        ...
        **params: Any,
        ...
        # Interpolate parameters (safe: reject attribute/index access)
        if params:
            try:
                # Use a safe format that rejects {0.__class__} and {key[index]}
                import string

                formatter = string.Formatter()
                # Validate that no field has attribute/index access
                has_unsafe_fields = False
                for _, field_name, _, _ in formatter.parse(message):
                    if field_name and ("." in field_name or "[" in field_name):
                        has_unsafe_fields = True
                        break
                if not has_unsafe_fields:
                    message = message.format_map(collections.defaultdict(str, params))
            except KeyError:
