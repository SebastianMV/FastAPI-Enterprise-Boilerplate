...
class CachedTenantRepository:
    ...
    async def get_by_id(self, tenant_id: UUID) -> Optional[Tenant]:
        ...
        if cached:
            logger.debug(f"Cache hit: tenant {tenant_id}")
    ...
    async def get_by_slug(self, slug: str) -> Optional[Tenant]:
        ...
        cached = await cache.get(cache_key)
        if cached:
            logger.debug(f"Cache hit: tenant slug {slug}")
            return self._dict_to_tenant(cached)
    ...
    async def get_by_domain(self, domain: str) -> Optional[Tenant]:
        ...
        cached = await cache.get(cache_key)
        if cached:
            logger.debug(f"Cache hit: tenant domain {domain}")
            return self._dict_to_tenant(cached)
    ...
    def _tenant_to_dict(tenant: Tenant) -> dict:
        ...
        return {
            ...
            "plan": tenant.plan,
            "plan_expires_at": (
                tenant.plan_expires_at.isoformat()
