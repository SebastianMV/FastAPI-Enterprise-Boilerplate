...
class TestDatabaseCLICommands:
    ...
    def test_database_seed_without_clear(self, mock_confirm: MagicMock, mock_run: MagicMock) -> None:
        """Test database seed without clear flag."""
        mock_run.return_value = None
        from app.cli.commands.database import app
        result = runner.invoke(app, ["seed", "--no-users", "--no-roles", "--no-tenants"])
        # Should execute without confirmation since no --clear
        mock_run.assert_called_once()

    @patch("app.cli.commands.database.asyncio.run")
    @patch("app.cli.commands.database.confirm_action", return_value=True)
    def test_database_seed_with_clear_confirmed(self, mock_confirm: MagicMock, mock_run: MagicMock) -> None:
    ...
    def test_database_seed_with_clear_rejected(self, mock_confirm: MagicMock, mock_run: MagicMock) -> None:
...
class TestAPIKeysCLICommands:
    ...
    def test_apikeys_revoke_confirmed(self, mock_confirm: MagicMock, mock_run: MagicMock) -> None:
