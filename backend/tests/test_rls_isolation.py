...
class RLSIsolationTester:
    ...
    async def setup_test_data(self, conn: asyncpg.Connection) -> bool:
        ...
        try:
            ...
            password_hash = (
                "$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5aW.u6CnP6W0K"
            )
        ...
        try:
            ...
            async with get_db_context() as session:
                ...
                if len(tenant_ids) == 1 and str(self.tenant_a_id) in tenant_ids:
                    print_success(
                        f"SQLAlchemy: Tenant A solo ve sus usuarios ({len(users_a)} usuarios)"
                    )
                    self.passed_tests += 1
                else:
                    print_error(
                        f"SQLAlchemy: Tenant A ve múltiples tenants: {tenant_ids}"
                    )
                    self.failed_tests += 1
            ...
            async with get_db_context() as session:
                ...
                if len(tenant_ids) == 1 and str(self.tenant_b_id) in tenant_ids:
                    print_success(
                        f"SQLAlchemy: Tenant B solo ve sus usuarios ({len(users_b)} usuarios)"
                    )
                    self.passed_tests += 1
                else:
                    print_error(
                        f"SQLAlchemy: Tenant B ve múltiples tenants: {tenant_ids}"
                    )
                    self.failed_tests += 1
    ...
    async def test_database_rls_direct(self, conn: asyncpg.Connection) -> None:
        ...
        try:
            ...
            await conn.execute(
                f"SET LOCAL app.current_tenant_id = '{self.tenant_a_id}';"
            )
            users_a = await conn.fetch(
                "SELECT id, email, tenant_id FROM users WHERE email LIKE 'rls-%';"
            )
            ...
            if len(tenant_ids) == 1 and str(self.tenant_a_id) in tenant_ids:
                print_success(
                    f"BD Direct: Tenant A solo ve sus usuarios ({len(users_a)} usuarios)"
                )
                self.passed_tests += 1
            else:
            ...
            await conn.execute("BEGIN;")
            await conn.execute(
                f"SET LOCAL app.current_tenant_id = '{self.tenant_b_id}';"
            )
            users_b = await conn.fetch(
                "SELECT id, email, tenant_id FROM users WHERE email LIKE 'rls-%';"
            )
            await conn.execute("COMMIT;")
            ...
            if len(tenant_ids) == 1 and str(self.tenant_b_id) in tenant_ids:
                print_success(
                    f"BD Direct: Tenant B solo ve sus usuarios ({len(users_b)} usuarios)"
                )
                self.passed_tests += 1
            else:
    ...
    async def test_rls_policies(self, conn: asyncpg.Connection) -> None:
        ...
        try:
            ...
            if len(policies) >= 9:
                ...
                print_error(
                    f"Solo {len(policies)} políticas encontradas, esperadas >= 9"
                )
                self.failed_tests += 1
        except Exception as e:
    ...
    def print_summary(self) -> None:
        ...
        if self.failed_tests == 0:
            ...
            print(
                f"{Colors.GREEN}Aislamiento multi-tenant RLS funcionando correctamente{Colors.RESET}"
            )
            print(
                f"{Colors.GREEN}Defensa en profundidad: SQLAlchemy Events + PostgreSQL RLS{Colors.RESET}"
            )
        else:
            print(
    ...
    async def run(self) -> bool:
        try:
            ...
            print_info(
                "Modo: SQLAlchemy Events + PostgreSQL RLS (Defensa en Profundidad)"
            )
            ...
            print_info("Setup con: boilerplate (owner - bypass RLS)")
            ...
            print_info("Tests con: app_user (NO owner - RLS activo)")
