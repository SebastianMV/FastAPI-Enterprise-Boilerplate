...
class TestCheckDatabase:
    ...
    async def test_check_database_success(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker', return_value=mock_session):
    ...
    async def test_check_database_failure(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker', side_effect=Exception("DB error")):
            result = await check_database()
...
class TestCheckRedis:
    ...
    async def test_check_redis_success(self):
        ...
        with patch('app.infrastructure.cache.redis_client.get_redis_client', return_value=mock_redis):
    ...
    async def test_check_redis_failure(self):
        ...
        with patch('app.infrastructure.cache.redis_client.get_redis_client', side_effect=Exception("Redis error")):
            result = await check_redis()
...
class TestGetOrCreateDefaultTenant:
    ...
    async def test_get_existing_tenant(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker', return_value=mock_session), \
             patch('app.infrastructure.database.repositories.tenant_repository.SQLAlchemyTenantRepository', return_value=mock_repo):
