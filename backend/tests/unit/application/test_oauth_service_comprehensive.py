...
class TestInitiateOAuth:
    ...
    async def test_initiate_oauth_generates_state_and_url(self, oauth_service):
        ...
        mock_provider.get_authorization_url = MagicMock(return_value="https://oauth.com/auth")
        mock_provider.redirect_uri = "https://app.com/callback"
        
        with patch.object(oauth_service, '_get_provider', return_value=mock_provider), \
    ...
    async def test_initiate_oauth_with_custom_redirect(self, oauth_service):
        ...
        with patch.object(oauth_service, '_get_provider', return_value=mock_provider), \
             patch.object(oauth_service, '_store_oauth_state', new_callable=AsyncMock) as mock_store:
...
class TestHandleCallback:
    ...
    async def test_handle_callback_success_new_user(
        ...
        with patch.object(oauth_service, '_get_oauth_state', return_value=state_data), \
             ...
             patch.object(oauth_service, '_find_or_create_user', new_callable=AsyncMock, return_value=(sample_user, mock_connection, True)):
            ...
            mock_provider.exchange_code.assert_called_once_with(code="auth_code", code_verifier="verifier")
    ...
    async def test_handle_callback_linking_existing_user(
        ...
        with patch.object(oauth_service, '_get_oauth_state', return_value=state_data), \
             patch.object(oauth_service, '_delete_oauth_state', new_callable=AsyncMock), \
             patch.object(oauth_service, '_get_provider', return_value=mock_provider), \
             patch.object(oauth_service, '_link_oauth_account', new_callable=AsyncMock, return_value=(sample_user, mock_connection)):
...
class TestFindOrCreateUser:
    ...
    async def test_find_existing_oauth_connection(
        ...
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=existing_connection), \
             patch.object(oauth_service, '_update_oauth_connection', new_callable=AsyncMock), \
             patch.object(oauth_service, '_get_user_by_id', new_callable=AsyncMock, return_value=sample_user):
    ...
    async def test_create_new_user_from_oauth(
        ...
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=None), \
             patch.object(oauth_service, '_get_user_by_email', new_callable=AsyncMock, return_value=None), \
             patch.object(oauth_service, '_create_user_from_oauth', new_callable=AsyncMock, return_value=new_user), \
             patch.object(oauth_service, '_create_oauth_connection', new_callable=AsyncMock, return_value=mock_connection):
...
class TestLinkOAuthAccount:
    ...
    async def test_link_oauth_to_user(
        ...
        with patch.object(oauth_service, '_get_user_by_id', new_callable=AsyncMock, return_value=sample_user), \
             patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=None), \
             patch.object(oauth_service, '_create_oauth_connection', new_callable=AsyncMock, return_value=mock_connection):
...
class TestHelperMethods:
    ...
    async def test_get_provider_with_sso_config(self, oauth_service):
        ...
        with patch.object(oauth_service, 'get_sso_config', new_callable=AsyncMock, return_value=[mock_sso_config]), \
             patch('app.application.services.oauth_service.get_oauth_provider') as mock_get_provider:
    ...
    async def test_get_provider_without_sso_config(self, oauth_service):
        ...
        with patch.object(oauth_service, 'get_sso_config', new_callable=AsyncMock, return_value=[]), \
             patch('app.application.services.oauth_service.get_oauth_provider') as mock_get_provider:
    ...
    @pytest.mark.asyncio
    async def test_link_oauth_already_linked_to_same_user(self, oauth_service, sample_user, sample_oauth_tokens, sample_oauth_user_info):
        """Test linking OAuth when already linked to same user."""
        mock_connection = MagicMock()
        mock_connection.user_id = sample_user.id
        
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=mock_connection), \
             patch.object(oauth_service, '_get_user_by_id', new_callable=AsyncMock, return_value=sample_user):
    ...
    @pytest.mark.asyncio
    async def test_link_oauth_already_linked_to_different_user(self, oauth_service, sample_user, sample_oauth_tokens, sample_oauth_user_info):
        ...
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=mock_connection):
            with pytest.raises(ValueError, match="This OAuth account is linked to another user"):
    ...
    @pytest.mark.asyncio
    async def test_find_or_create_user_with_existing_email(self, oauth_service, sample_user, sample_oauth_tokens, sample_oauth_user_info):
        """Test finding user when email already exists."""
        mock_connection = MagicMock()
        
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=None), \
             patch.object(oauth_service, '_get_user_by_email', new_callable=AsyncMock, return_value=sample_user), \
             patch.object(oauth_service, '_create_oauth_connection', new_callable=AsyncMock, return_value=mock_connection):
    ...
    async def test_unlink_primary_oauth_without_password(self, oauth_service):
        ...
        oauth_service._session.execute = AsyncMock(side_effect=[mock_result, mock_user_result])
        
        with pytest.raises(ValueError, match="Cannot unlink primary OAuth account without setting a password"):
...
class TestPrivateMethods:
    ...
    @pytest.mark.asyncio
    async def test_create_oauth_connection_with_all_data(self, oauth_service, sample_oauth_tokens, sample_oauth_user_info):
    ...
    @pytest.mark.asyncio
    async def test_create_user_from_oauth_with_full_name(self, oauth_service, sample_oauth_user_info):
    ...
    @pytest.mark.asyncio
    async def test_create_user_from_oauth_split_full_name(self, oauth_service, sample_oauth_user_info):
    ...
    @pytest.mark.asyncio
    async def test_create_user_from_oauth_email_fallback(self, oauth_service, sample_oauth_user_info):
    ...
    @pytest.mark.asyncio
    async def test_find_or_create_user_not_found_error(self, oauth_service, sample_oauth_tokens, sample_oauth_user_info):
        ...
        with patch.object(oauth_service, '_get_oauth_connection', new_callable=AsyncMock, return_value=mock_connection), \
             patch.object(oauth_service, '_update_oauth_connection', new_callable=AsyncMock), \
             patch.object(oauth_service, '_get_user_by_id', new_callable=AsyncMock, return_value=None):
            
            with pytest.raises(ValueError, match="User not found for OAuth connection"):
    ...
    @pytest.mark.asyncio
    async def test_link_oauth_user_not_found_error(self, oauth_service, sample_oauth_tokens, sample_oauth_user_info):
