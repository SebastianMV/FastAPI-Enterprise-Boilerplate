...
class TestAppConfiguration:
    ...
    def test_app_includes_api_router(self) -> None:
        ...
        api_routes = [r for r in app.routes if hasattr(r, 'path') and '/api/v1' in str(r.path)]  # type: ignore[attr-defined]
...
class TestLifespan:
    ...
    async def test_lifespan_startup_and_shutdown(self) -> None:
        ...
        with patch("app.infrastructure.database.connection.init_database", new_callable=AsyncMock) as mock_init_db:
            with patch("app.infrastructure.database.connection.close_database", new_callable=AsyncMock) as mock_close_db:
                with patch("app.infrastructure.observability.logging.setup_logging") as mock_setup_logging:
...
class TestCORSConfiguration:
    ...
    def test_cors_middleware_added(self) -> None:
        ...
        middleware_names = [m.cls.__name__ for m in app.user_middleware if hasattr(m, 'cls')]  # type: ignore[attr-defined]
