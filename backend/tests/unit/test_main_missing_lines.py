...
class TestLifespanOpenTelemetry:
    ...
    async def test_lifespan_otel_enabled_success(self) -> None:
        ...
        with patch("app.infrastructure.database.connection.init_database", new_callable=AsyncMock):
            with patch("app.infrastructure.database.connection.close_database", new_callable=AsyncMock):
                with patch("app.infrastructure.observability.logging.setup_logging"):
                    with patch("app.infrastructure.observability.telemetry.setup_telemetry") as mock_setup_telemetry:
...
class TestLifespanUptimeTrackerError:
    ...
    async def test_lifespan_uptime_tracker_initialization_error(self) -> None:
        ...
        with patch("app.infrastructure.database.connection.init_database", new_callable=AsyncMock):
            with patch("app.infrastructure.database.connection.close_database", new_callable=AsyncMock):
                with patch("app.infrastructure.observability.logging.setup_logging"):
                    with patch("app.infrastructure.monitoring.get_uptime_tracker") as mock_get_uptime:
                        # Make uptime tracker initialization fail
                        mock_tracker = AsyncMock()
                        mock_tracker.initialize = AsyncMock(side_effect=Exception("Uptime tracker init failed"))
