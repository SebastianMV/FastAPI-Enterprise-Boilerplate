...
class TestSeedDatabaseCommand:
    ...
    async def test_seed_database_creates_tenants(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker') as mock_maker, \
             patch('app.infrastructure.database.repositories.tenant_repository.SQLAlchemyTenantRepository', return_value=mock_tenant_repo), \
             patch('app.infrastructure.database.repositories.role_repository.SQLAlchemyRoleRepository', return_value=mock_role_repo), \
             patch('app.infrastructure.database.repositories.user_repository.SQLAlchemyUserRepository', return_value=mock_user_repo), \
            ...
            await _seed_database(include_users=False, include_roles=False, include_tenants=True, clear_existing=False)
    ...
    async def test_seed_database_creates_roles(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker') as mock_maker, \
            ...
            await _seed_database(include_users=False, include_roles=True, include_tenants=False, clear_existing=False)
    ...
    async def test_seed_database_creates_users(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker') as mock_maker, \
             ...
             patch('app.infrastructure.database.repositories.role_repository.SQLAlchemyRoleRepository', return_value=mock_role_repo), \
             patch('app.infrastructure.database.repositories.user_repository.SQLAlchemyUserRepository', return_value=mock_user_repo), \
             patch('app.infrastructure.auth.jwt_handler.hash_password', return_value='hashed'):
            
            mock_maker.return_value.__aenter__.return_value = mock_session
            mock_maker.return_value.__aexit__.return_value = None
            
            await _seed_database(include_users=True, include_roles=False, include_tenants=False, clear_existing=False)
    ...
    async def test_seed_database_creates_default_tenant_when_needed(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker') as mock_maker, \
             ...
             patch('app.infrastructure.database.repositories.role_repository.SQLAlchemyRoleRepository', return_value=mock_role_repo), \
             patch('app.infrastructure.database.repositories.user_repository.SQLAlchemyUserRepository', return_value=mock_user_repo), \
             patch('app.cli.utils.get_or_create_default_tenant', return_value=default_tenant_id) as mock_default:
...
class TestDatabaseInfoCommand:
    ...
    async def test_database_info_handles_connection_error(self):
        ...
        with patch('app.infrastructure.database.connection.async_session_maker') as mock_maker:
            mock_maker.return_value.__aenter__.side_effect = Exception("Connection failed")
...
class TestRunMigrationsCommand:
    ...
    def test_run_migrations_default_revision(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            run_migrations(revision="head")  # Explicitly pass head instead of using default
...
class TestResetDatabaseCommand:
    ...
    async def test_reset_database_drops_and_recreates_schema(self):
        ...
        with patch('app.infrastructure.database.connection.engine') as mock_engine, \
            ...
            mock_subprocess.assert_called_once_with(["alembic", "upgrade", "head"], check=True)
