...
class TestRefreshTokenEndpoint:
    ...
    async def test_refresh_token_invalid(self, mock_session: MagicMock) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc:
                await refresh_token(
                    request=request, session=mock_session, http_request=mock_request
                )
                                    ...
                                    with patch(
                                        ...
                                        with patch(
                                            ...
                                            with patch(
                                                ...
                                                # Compatible with both AuthResponse(tokens) and TokenResponse
...
class TestEmailVerificationEndpoints:
    ...
    async def test_verify_email_invalid_token(self, mock_session: MagicMock) -> None:
        ...
        with pytest.raises(HTTPException) as exc:
            await verify_email(
                request=VerifyEmailTokenRequest(token=token), session=mock_session
            )
