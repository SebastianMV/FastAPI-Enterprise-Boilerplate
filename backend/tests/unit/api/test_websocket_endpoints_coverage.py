...
class TestGetWsManager:
    ...
    def test_get_ws_manager_singleton(self) -> None:
        """Test WebSocket manager is cached as singleton."""
        import app.api.v1.endpoints.websocket as ws_module
...
class TestDefaultHandlers:
    ...
    async def test_handle_NOTIFICATION_to_room(self) -> None:
        ...
        recipient_connection_id = await manager.connect(
            recipient_ws, recipient_user_id, None
        )
...
class TestWebSocketEndpointErrorHandling:
    ...
    async def test_websocket_message_processing_error(self) -> None:
        ...
        with (
            patch(
                "app.api.v1.endpoints.websocket.get_ws_manager",
