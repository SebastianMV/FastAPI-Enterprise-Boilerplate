    ...
    async def test_create_user_conflict_error(self, mock_session):
        ...
        with (
            ...
            mock_repo.create.side_effect = ConflictError(
                code="DUPLICATE_KEY", message="Duplicate entry"
            )
            mock_repo_cls.return_value = mock_repo
...
class TestRolesEndpointCoverage:
    ...
    async def test_create_role_conflict_error(self, mock_session):
        ...
        with (
            ...
            mock_repo = AsyncMock()
            mock_repo.create.side_effect = ConflictError(
                code="DUPLICATE_ROLE", message="Role already exists"
            )
            mock_repo_cls.return_value = mock_repo
...
class TestTenantsEndpointCoverage:
    ...
    async def test_update_tenant_not_found(self):
        ...
        with pytest.raises(HTTPException) as exc:
            await update_tenant(
                tenant_id=tenant_id, data=data, current_user_id=uuid4(), repo=mock_repo
            )
...
class TestTelemetryCoverage:
    ...
    def test_setup_tracing_with_otlp_endpoint(self):
        ...
        with (
            ...
            patch(
                "app.infrastructure.observability.telemetry.TracerProvider"
            ) as mock_provider,
            patch(
                "app.infrastructure.observability.telemetry.OTLPSpanExporter"
            ) as mock_exporter,
            patch(
                "app.infrastructure.observability.telemetry.BatchSpanProcessor"
            ) as mock_processor,
    ...
    def test_setup_tracing_without_otlp_endpoint(self):
        ...
        with (
            ...
            patch(
                "app.infrastructure.observability.telemetry.ConsoleSpanExporter"
            ) as mock_console,
            patch(
                "app.infrastructure.observability.telemetry.BatchSpanProcessor"
            ) as mock_processor,
    ...
    def test_setup_metrics_with_otlp_endpoint(self):
        ...
        with (
            ...
            patch(
                "app.infrastructure.observability.telemetry.MeterProvider"
            ) as mock_provider,
            patch(
                "app.infrastructure.observability.telemetry.OTLPMetricExporter"
            ) as mock_exporter,
            patch(
                "app.infrastructure.observability.telemetry.PeriodicExportingMetricReader"
            ) as mock_reader,
    ...
    def test_setup_metrics_without_otlp_endpoint(self):
        ...
        with (
            ...
            patch(
                "app.infrastructure.observability.telemetry.ConsoleMetricExporter"
            ) as mock_console,
            patch(
                "app.infrastructure.observability.telemetry.PeriodicExportingMetricReader"
            ) as mock_reader,
