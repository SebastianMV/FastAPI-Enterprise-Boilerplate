    ...
    async def test_update_role_entity_not_found_error(self, mock_session):
        ...
        with patch(
            ...
            mock_repo.update.side_effect = EntityNotFoundError(
                code="ROLE_NOT_FOUND", message="Role not found during update"
            )
            mock_repo_cls.return_value = mock_repo
...
class TestMFADisable:
    ...
    @pytest.mark.asyncio
    async def test_disable_mfa_password_verification_fails(self, mock_session):
        """Test disable_mfa raises 401 when password is invalid."""
        from fastapi import HTTPException
        ...
        # verify_password returns False to trigger the 401 error
    ...
    @pytest.mark.asyncio
    async def test_disable_mfa_success_with_valid_password(self, mock_session):
        """Test disable_mfa succeeds with valid password and code."""
        from app.api.v1.endpoints.mfa import (
            disable_mfa,
        )
        from app.api.v1.schemas.mfa import MFADisableRequest
...
class TestRolesAdditionalCoverage:
    ...
    async def test_create_role_conflict_error(self, mock_session):
        ...
        with patch(
            ...
            mock_repo.create.side_effect = ConflictError(
                code="ROLE_EXISTS", message="Role already exists"
            )
            mock_repo_cls.return_value = mock_repo
...
class TestRolesAdditionalFlows:
    ...
    async def test_update_role_with_invalid_permission(self, mock_session):
        ...
        with (
            ...
            mock_permission.from_string.side_effect = ValueError(
...
class TestUsersCreateSuccess:
    ...
    async def test_create_user_success_path(self, mock_session):
        ...
        mock_created_user.updated_at = datetime.now(UTC)
...
class TestTenantsSlugDomainExists:
    ...
    async def test_update_tenant_slug_exists_raises(self):
        ...
        assert exc.value.status_code == 409
        assert (
            "slug" in str(exc.value.detail).lower()
    ...
    async def test_update_tenant_domain_exists_raises(self):
        ...
        assert exc.value.status_code == 409
        assert (
            "domain" in str(exc.value.detail).lower()
            or "exists" in str(exc.value.detail).lower()
        )
...
class TestAuthEmailExceptionPaths:
    ...
    async def test_send_verification_email_exception_silent(self, mock_db_session):
        ...
        mock_user.first_name = "Test"
        mock_user.email_verified = False
        mock_user.generate_email_verification_token = MagicMock(
...
class TestRolesDescriptionCoverage:
    ...
    async def test_update_role_description(self, mock_session):
        ...
        updated_role.created_at = datetime.now(UTC)
        updated_role.updated_at = datetime.now(UTC)
