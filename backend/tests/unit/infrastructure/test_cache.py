...
class TestRedisCache:
    ...
    async def test_set_with_ttl(
        ...
        mock_redis_client.setex.assert_called_once_with(
            "session_key", ttl, json.dumps(data)
        )
...
class TestCacheKeyBuilder:
    ...
    def test_build_key_with_uuid(self) -> None:
        ...
        assert key == "user:12345678-1234-1234-1234-123456789abc"
...
class TestCacheServiceDisabled:
    ...
    def test_is_enabled_false_without_client(self, disabled_cache_service) -> None:
        """Test is_enabled returns False without client."""
        assert disabled_cache_service.is_enabled is False

    @pytest.mark.asyncio
    async def test_get_returns_none_when_disabled(self, disabled_cache_service) -> None:
...
class TestCacheServiceClearAll:
    ...
    async def test_clear_all_returns_false_on_error(
        ...
        with patch.object(
            cache_service, "delete_pattern", side_effect=Exception("Redis error")
        ):
            result = await cache_service.clear_all()
