...
--
-- docker-compose.deploy.yml already mounts init_prod.sh correctly.
...
) THEN RAISE EXCEPTION 'FATAL: Do not use init_prod.sql directly. ' 'Use init_prod.sh instead, which reads APP_USER_PASSWORD ' 'from the environment. See docker-compose.deploy.yml.';
END IF;
END $$;
DO $$ BEGIN RAISE NOTICE 'Production database initialized for FastAPI-Enterprise-Boilerplate';
END $$;
